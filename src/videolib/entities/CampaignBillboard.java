...
public final class CampaignBillboard extends CustomCampaignEntityWrapper implements Serializable {
    ...
    private float lensStructureScale;
     ...
     * @return a {@link CampaignBillboard} Object wrapping a {@link CustomCampaignEntity}
     */
    ...
    private void setVFrameSprite(SpriteAPI sprite) {
    ...
    public void setDescription(String desc) {
        ...
        if (this.descriptionId == null) {
            this.descriptionId = UUID.randomUUID().toString() + "vl_billboard";
        }
        this.registerDescription();
    }

    private void registerDescription() {
        this.setCustomDescriptionId(this.descriptionId);

        Description desc = new Description(this.descriptionId, Description.Type.CUSTOM);
        desc.setText1(this.description);
        desc.setText2(this.description);
        
        Set<String> descIds = (Set<String>) Global.getSector().getPersistentData().get("vlBillboardDescriptions");
        if (descIds == null) {
            descIds = (Set<String>) Global.getSector().getPersistentData().computeIfAbsent("vlBillboardDescriptions", k -> new HashSet<>());
        }

        descIds.add(this.descriptionId);
        VideoLibModPlugin.addToDescriptionMap(this.descriptionId, desc);
    }
    ...
    private void setHoloLensStructureScale(float scale) {
        this.lensStructureScale = scale;
        this.ourSprite.lensStructure.setSize(this.ourSprite.origLensStructureHeight * scale, this.ourSprite.origLensStructureWidth * scale);

        this.ourSprite.lensFaceLength = 45f * scale;

        this.ourSprite.lensWidth = this.ourSprite.origLensStructureWidth / 2.75f * scale;
        this.ourSprite.lensHeight = this.ourSprite.origLensStructureHeight/ 2.75f * scale;

        this.ourSprite.sideLensWidth = this.ourSprite.lensWidth / 1.5f;
        this.ourSprite.sideLensHeight = this.ourSprite.lensHeight / 1.5f;
        this.ourSprite.sideLensOffset = this.ourSprite.lensStructure.getWidth() / 2 - 13f;
    }
    ...
    private class BillboardSprite extends Sprite {
        ...
        private float origLensStructureWidth = lensStructure.getWidth();
        private float origLensStructureHeight = lensStructure.getHeight();
        ...
        private float lensFaceLength;
        private float lensWidth;
        private float lensHeight;
        private float sideLensWidth;
        private float sideLensHeight;
        private float sideLensOffset;
        ...
        private void renderVframePseudo3DSkewed(float x, float y, float dist, float facingAngle) {
            ...
            self.vFrameSprite.bindTexture();
            GL11.glPushMatrix();
            GL11.glColor4ub((byte)this.color.getRed(), (byte)this.color.getGreen(), (byte)this.color.getBlue(),
                            (byte)((int)(this.color.getAlpha() * this.realVframeAlpha)));
    ...
    private void init() {
        ...
        this.setHoloLensStructureScale(this.lensStructureScale);
    ...
    private void ctorInit() {
        ...
        if (!(this.getPlugin() instanceof CampaignBillboardPlugin)) {
            throw new IllegalArgumentException("CustomCampaignEntityPlugin implementation for CampaignBillboard must be, or inherit from the class videolib.entities.CampaignBillboardPlugin. " + "Found: " 
                + this.getPlugin() == null ? "null" : this.getPlugin().getClass().getCanonicalName());
        }
        if (this.getCustomDescriptionId() != null) {
            throw new IllegalArgumentException("CampaignBillboard description should be set programmatically via CampaignBillboard.setDescription");
        }
